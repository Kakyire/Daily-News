...
class SportsScreen: UIViewController, UITableViewDelegate {
    ...
    @objc fileprivate func getNews() {
        networkRequest.getNews(for: K.sports, complete: { res in
...
extension SportsScreen: UITableViewDataSource{
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        if contentOffsetY + screenHeight >= contentHeight {
            ...
            if !isLoading {
                ...
                networkRequest.getNews(for: K.sports, page: currentPage, complete: {res in
                    self.isLoading = true

...
class HomeScreen: UIViewController, UITableViewDelegate{
    ...
    @objc fileprivate func getNews() {
        networkRequest.getNews(for: countryName(), complete:{res in
            self.handle(response: res,progressIndicator: self.activityIndicator,onSuccess: {news in
                self.articles = news.articles
                print("TotalArticles \(self.articles.count)")
    ...
    override func loadView() {
        super.loadView()
        setup(tableView: tableView,progressIndicator: activityIndicator,tableViewFooter: footerView)
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        if contentOffsetY + screenHeight >= contentHeight {
            ...
            if !isLoading {
                ...
                networkRequest.getNews(for: countryName(), page: currentPage, complete: {res in
                    self.isLoading = true
                    self.handle(response: res,progressIndicator: self.activityIndicator,onSuccess: {news in
                        self.articles += news.articles
                        self.tableView.reloadData()
